...
"приглашаем актеров" и т.д. Т.е. контекст сообщения должен описывать участие в 
съемках кино, сериалов, рекламы, некоммерческих проектов. От этого зависят жизни людей!
...
prob_prompt_text = """"Тебе будет поступать информация о кастинга для актеров. Тебе нужно проверить наличие текста для
самопроб в этом сообщении. Текст для самопроб - это  Обычно, он указывается в сообщении в виде описания сцены и последующего диалога.
Т.е. если текст для самопроб и есть, то он ОБЯЗАТЕЛЬНО должен содержать одну или несколько реплик для актеров.
Текст для самопроб присылать в том виде, как они есть в тексте сообщения. Чаще всего, этот текст начинается после фразы
"текст для самопроб" или что то похожее. Если такого текста в сообщении нет, то просто скажи "Отсутствует" . Описание роли НЕ 
является текстом для самопроб! Учти это. Придумывать текст для самопроб НЕЛЬЗЯ! Он либо есть в тексте сообщения, 
либо нет! Отвечай как описано в подсказке по форматированию 
{format_instructions}. Информация о кастинге: {input}
"""
prob_prompt_text_2 = """Извлеките текст для самопробы из сообщений о кастинге. 
Если текст для самопробы отсутствует, оставьте пустым. Будьте доскональны и выделите только самопробы.
