...
class ConfigurationParsing(BaseModel):
    ...
    project_type: str = Field(default='Unspecified',
                              description='Тип проекта. Реклама принимает значение "ads"; кино, фильмы и сериалы '
                                          'значение "films"; Некоммерческие проекты / фестивальные короткометражные '
    ...
    fee: int = Field(default='Unspecified', description='Гонорар за роль. В сообщении указан диапазон, '
...
class ContactsParsing(BaseModel):
    ...
    contacts: str = Field(default='комментарии', description='Контакты, которые оставил автор кастинга для приема заявок. '
...
обязательно учти это, то очень важно. 
