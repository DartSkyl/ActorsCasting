...
Текст сообщения: {input}"""
check_prompt_text_ = """Пользователь отправляет  тебе пост с исходным текстом,  ты должен определить, является ли этот 
пост кастингом или нет. Если является, то проанализируй его и преобразуй в новый пост по указанным правилам и образцу.
Шаги обработки:
Определи, является ли пост кастингом.
Критерии, по которым пост считается кастингом:
Пост содержит формулировки типа "Ищем актёров...", "Требуются модели...", "Проводится кастинг...".
Описание ролей или позиций: Указаны конкретные роли, типажи или позиции с требованиями к участникам (возраст, пол, внешность, навыки).
Информация о проекте: Присутствуют детали о проекте (название, тип проекта, даты съёмок или проведения, гонорар).
Контактные данные: Предоставлены способы связи для отправки заявок (email, телефон, ссылки на анкеты или формы).
Инструкции по подаче заявки: Есть указания, как подать заявку, какие материалы предоставить (фотографии, резюме, видеопробы).
Пост НЕ является кастингом, если:
Реклама или промоакция: Это реклама услуг, мероприятий, вебинаров, тренингов, курсов, ботов или программ.
Информационное сообщение: Сообщение носит общий характер без приглашения к участию (например, новости, отчёты, благодарности, поздравления).
Отсутствие конкретики: Нет конкретных требований к участникам, отсутствует информация о наборе на роли.
Объявления о результатах: Сообщения о завершении кастинга, благодарности за участие, объявления о переносе дат без приглашения новых участников.
...
async def check_words(text: str):
    ...
    words = ["викторина", "выиграть", "викторине"]
    for word in words:
